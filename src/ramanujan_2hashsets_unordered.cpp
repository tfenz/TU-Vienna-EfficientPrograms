...
int main(int argc, char **argv) {
    ...
    std::unordered_set<long> *candidates = new std::unordered_set<long>[bound];
...
//    long total_collision_count = 0; //todo debug only
    for (i = 0; cube(i) <= n; i++) {
        for (j = i + 1; cube(i) + cube(j) <= n; j++) {
            ...
            auto res = candidates[idx].insert(current_candidate);
            if (!res.second){
                rama_numbers.insert(current_candidate);
            }
